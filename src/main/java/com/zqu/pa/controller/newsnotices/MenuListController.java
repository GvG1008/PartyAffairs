...
public class MenuListController {
     ...
     */
    @ResponseBody
    @RequestMapping("/newsMenu/{pageNum}")
    public ServerResponse<PageOfList> getNewsMenu(@PathVariable(value="pageNum") int page){
        PageOfList info = new PageOfList();
        
        //传入跳转的页数与当前显示的条数
        info = newsService.getMenuInfo(page,10);
     ...
     */
    @ResponseBody
    @RequestMapping("/noticesMenu/public/{pageNum}")
    public ServerResponse<PageOfList> getPublicMenu(@PathVariable(value="pageNum") int page){
        PageOfList info = new PageOfList();
        
        //传入跳转的页数与当前显示的条数
        info = noticesService.getMenuInfo(page,10,0);
     ...
     */
    @ResponseBody
    @RequestMapping("/noticesMenu/party/{pageNum}")
    public ServerResponse<PageOfList> getPartyMenu(@PathVariable(value="pageNum") int page){
        PageOfList info = new PageOfList();
        
        //传入跳转的页数与当前显示的条数
        info = noticesService.getMenuInfo(page,10,1);
